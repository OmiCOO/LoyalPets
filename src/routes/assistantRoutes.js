...
async function initializeAssistant() {
    try {
        ...
        const retrievedAssistant = await openai.beta.assistants.retrieve(assistant.id);
...
router.post('/message', async (req, res) => {
    ...
    try {
        ...
        const run = await openai.beta.threads.runs.create(threadId, {
            assistant_id: assistant.id,
            instructions: `Consider this pet's information while responding: ${JSON.stringify(petInfo)}`
        });
        ...
        while (attempts < maxAttempts) {
            ...
            if (runStatus.status === 'failed') {
                ...
                throw new Error(`Run failed with status: ${runStatus.status}. Last error: ${runDetails.last_error?.message || 'Unknown error'}`);
            }
        ...
        if (runStatus.status !== 'completed') {
            throw new Error(`Assistant response timed out after ${maxAttempts} seconds. Final status: ${runStatus.status}`);
        }
        ...
        if (uncertaintyIndicators.some(indicator => response.toLowerCase().includes(indicator.toLowerCase()))) {
            ...
            if (searchResults && searchResults.length > 0) {
                ...
                finalResponse = `Based on veterinary sources, here's what I found:\n\n${result.content}\n\n` +
...
async function needsUpdate(petId) {
    ...
    const result = await pool.query(
        'SELECT last_updated FROM pets WHERE id = $1',
        [petId]
    );
